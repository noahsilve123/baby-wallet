...
import { ArrowRight, BookOpen, ChevronRight, GraduationCap, Heart, Sparkles, Users } from 'lucide-react'
...
export default function Page() {
	const quickLinks = [
		{
			...
			label: 'Get support',
			icon: <GraduationCap className="h-5 w-5" />, // icon inlined to avoid ref typing
		},
		{
	...
	const pathways = [
		{
			title: '1. Plan',
			description: 'Personal roadmap beginning in 9th grade with family meetings and timeline planning.',
		},
		{
			title: '2. Practice',
			description: 'SAT cohorts, essay studios, interview prep, and financial aid nights led by counselors.',
		},
		{
			title: '3. Prevail',
			description: 'Scholarship packaging, transition-to-campus coaching, and alumni mentoring.',
		},
	]
		...
		<main className="bg-white text-gray-900">
			...
			<section className="relative overflow-hidden bg-gradient-to-br from-rose-50 via-white to-amber-50 text-gray-900">
				...
				<div className="relative max-w-7xl mx-auto px-6 py-20 grid gap-12 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)] items-center">
					...
					<div className="glass-panel rounded-3xl p-8 text-gray-900">
						<div className="flex items-center gap-3 text-sm font-semibold text-gray-700">
							<BookOpen className="h-5 w-5" /> Weekly focus plan
						</div>
						<p className="mt-4 text-xl font-semibold">&ldquo;I never imagined having a mentor who understood each deadline. Now my FAFSA is filed and I&rsquo;m visiting campuses.&rdquo;</p>
				...
				<div className="max-w-6xl mx-auto px-6">
					<div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
						...
						<div className="rounded-3xl bg-gray-900 p-8 text-white shadow-xl">
							...
							<Link href="/resources" className="mt-8 inline-flex items-center rounded-full bg-white px-5 py-2 text-sm font-semibold text-gray-900">
			...
			<section className="bg-slate-900 text-white">
				<div className="max-w-6xl mx-auto px-6 py-16 text-center">
					...
					<div className="mt-8 flex flex-wrap justify-center gap-4">
						<Link href="/donate" className="btn-crimson inline-flex items-center gap-2 rounded-full px-6 py-3 font-semibold text-white">
							Fuel scholarships <ArrowRight className="h-4 w-4" />
						</Link>
						<Link href="/programs" className="inline-flex items-center gap-2 rounded-full border border-white/40 px-6 py-3 font-semibold text-white">
							Refer a student
						</Link>
					</div>
				</div>
			...
			<footer className="bg-gray-50 border-t">
				<div className="max-w-7xl mx-auto px-6 py-6 text-center text-sm text-gray-600">
					© {new Date().getFullYear()} Destination College — All rights reserved.
				</div>
			</footer>
		</main>
	)
